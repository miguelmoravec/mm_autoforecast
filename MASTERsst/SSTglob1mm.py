...
#This script automatically generates 6 forecast plots of Global SST anomalies by averaging model predictions monthly for 11 months succeeding a specified date and then comparing the averages to historical climatologies.
...
def mymain(argv):    
	...
	while (count < 6): 
		...
		cmd2 = 'SHADE/SET_UP/lev=(-inf)(-3,-1,1)(-1,-0.5)(-0.5,0.5,0.25)(0.5,1,0.5)(1,3,1)(inf)/PALETTE=blue_darkred (temp[d=2,L=' + str(month_c) + ',K=1]-temp[d=1,L=' + str(month_c) + ',K=1])'				#equation essentially subtracts contemporary temp from historical temp at depth 5m (k=1) for a given month (l)
		...
		if count == 6:						#title
			...
			cmd11 = str('ANNOTATE/NOUSER/XPOS=-7.6/YPOS=5.25 "SST Global Anomalies ' + month + '/' + year + '-' + month_fut + '/' + year_fut + ' (Averaged Monthly)"')
	...
	if os.path.exists(filename):
		print 'SUCCESS. Plot image file for monthly global SST anom for ', month, '/', year, '-', month_fut, '/', year_fut, ' is located in the local directory and is named: ', filename
	else:
