...
#This script automatically generates 6 forecast plots of Global Precipitation anomalies by averaging model predictions every 3 months for 11 months succeeding a specified date and then comparing the averages to historical climatologies.
...
def mymain(argv):    
	...
	filename = 'precip_glob_anom_3mm_'+ str(date_abrev) + '.png'
	...
	while (count < 6): 
		...
		cmd2 = 'SHADE/SET_UP/lev=(-inf)(-10,-6,4)(-6,-2,2)(-2,-1,1)(-1,-0.5,0.5)(-0.5,0.5,0.25)(0.5,1,0.5)(1,2,1)(2,6,2)(6,10,4)(inf)/PALETTE=darkred_blue (86400*(precip[d=2,L=' + str(month_c) + ':' + str(month_c+1) + '@AVE]-precip[d=1,L=' + str(month_c) + ':' + str(month_c+1) + '@AVE]))'		
		...
		if count == 6:						#title
			...
			cmd11 = str('ANNOTATE/NOUSER/XPOS=-7.6/YPOS=5.25 "Precip Global Anomalies ' + month + '/' + year + '-' + month_fut + '/' + year_fut + ' (3 Month Averages)"')
	...
	if os.path.exists(filename):
		print 'SUCCESS. Plot image file for tri-monthly global precip anom for ', month, '/', year, '-', month_fut, '/', year_fut, ' is located in the local directory and is named: ', filename
	else:
