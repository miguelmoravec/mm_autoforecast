...
def mymain(argv):    
	...
	for opt, arg in opts:
 		if opt == '-h': #help option
        		print '\nThis script automatically generates 6 forecast plots of T ref anomalies by averaging model predictions monthly for 11 months succeeding a specified date and then comparing the averages to historical climatologies. \n'
	...
	filename = 'Tref_glob_anom_1mm_'+ str(date_abrev) + '.png'
	...
	while (count < 6): 
		...
		if count == 6:						#title
			...
			cmd11 = str('ANNOTATE/NOUSER/XPOS=-7.6/YPOS=5.25 "T Ref Global Anomalies ' + month + '/' + year + '-' + month_fut + '/' + year_fut + ' (Averaged Monthly)"')
	...
	if os.path.exists(filename):
		print 'SUCCESS. Plot image file for monthly global T ref anom for ', month, '/', year, '-', month_fut, '/', year_fut, ' is located in the local directory and is named: ', filename
	else:
